...
if [ $TARGET_MODULE ];then
	echo "-----------------TARGET_MODULE = $TARGET_MODULE-----------------"
	echo "terraform apply -var-file "environments/${ENV}/${DIR_NAME}/variables.tfvars" -target ${TARGET_MODULE}"
  terraform apply -var-file "environments/${ENV}/${DIR_NAME}/variables.tfvars" -target ${TARGET_MODULE}
else
	echo "-----------------TARGET_MODULE IS NOT EXISTS-------------------"
	echo "terraform apply -var-file "environments/${ENV}/${DIR_NAME}/variables.tfvars""
  terraform apply -var-file "environments/${ENV}/${DIR_NAME}/variables.tfvars"
fi
