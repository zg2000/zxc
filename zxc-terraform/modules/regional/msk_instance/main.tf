...
locals {
  ...
  bigdata_msk_logs_s3_bucket = var.bigdata_msk_logs_s3_bucket

  front-kafka = try(local.yaml-config-data.kafka.front, {})
  back-kafka  = try(local.yaml-config-data.kafka.back, {})
...
module "front_msk_instance" {
  ...
  s3_logs_bucket  = local.bigdata_msk_logs_s3_bucket
...
module "back_msk_instance" {
  source = "../../../private-modules/aws-msk-kafka-cluster"
  create = try(local.back-kafka.create, false)
  tags   = var.tags
  name   = "${local.resource-prefix}-external-${local.env}-kafka"

  kafka_version              = try(local.back-kafka.config.kafka_version, "2.8.1")
  number_of_broker_nodes     = try(local.back-kafka.config.number_of_broker_nodes, null)
  enhanced_monitoring        = try(local.back-kafka.config.enhanced_monitoring, "PER_TOPIC_PER_PARTITION")
  broker_node_client_vpc_id  = local.vpc_id
  broker_node_client_subnets = local.private_subnet_ids
  broker_node_instance_type  = try(local.back-kafka.config.broker_node_instance_type, "kafka.t3.small")
  storage_volume_size        = try(local.back-kafka.config.storage_volume_size, 100)

  s3_logs_enabled = true
  s3_logs_bucket  = local.bigdata_msk_logs_s3_bucket
  s3_logs_prefix  = "logs/back-msk-logs"

  create_configuration            = try(local.back-kafka.config.create_configuration, false)
  configuration_server_properties = try(local.back-kafka.config.configuration_server_properties, null)
}